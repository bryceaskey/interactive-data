...
convertToTidy <- function(data, selectedSamples){
  ...
  for(sample in selectedSamples){
    sampleData <- data[, 1:4]
    meanColName <- paste(sample, "_FPKM", sep="")
    meanCol <- data[, colnames(data)==meanColName]
    seColName <- paste(sample, "_SE", sep="")
    seCol <- data[, colnames(data)==seColName]
    sampleData$FPKM <- meanCol
    sampleData$SE <- seCol
...
getGenotypeCols <- function(selectedSamples){
  ...
  if(sum(selectedSamples=="WT")==1){genotypeCols <- append(genotypeCols, 5)}
  if(sum(selectedSamples=="WT_PAOx")==1){genotypeCols <- append(genotypeCols, 7)}
  if(sum(selectedSamples=="Cyp79A2")==1){genotypeCols <- append(genotypeCols, 9)}
  return(genotypeCols)
}
...
getGenotypeCols2 <- function(selectedSamples){
  genotypeCols <- vector(mode="numeric", length=0)
  if(sum(selectedSamples=="WT")==1){genotypeCols <- append(genotypeCols, c(5, 6))}
  if(sum(selectedSamples=="WT_PAOx")==1){genotypeCols <- append(genotypeCols, c(7, 8))}
  if(sum(selectedSamples=="Cyp79A2")==1){genotypeCols <- append(genotypeCols, c(9, 10))}
  return(genotypeCols)
}