...
getGenotypeCols <- function(selectedSamples){
  ...
  if(sum(selectedSamples=="Col0_d3d")==1){genotypeCols <- append(genotypeCols, 4)}
  if(sum(selectedSamples=="hy5_d3d")==1){genotypeCols <- append(genotypeCols, 6)}
  if(sum(selectedSamples=="Col0_d3d_l1.5h")==1){genotypeCols <- append(genotypeCols, 8)}
  if(sum(selectedSamples=="hy5_d3d_l1.5h")==1){genotypeCols <- append(genotypeCols, 10)}
  return(genotypeCols)
}
...
getGenotypeCols2 <- function(selectedSamples){
  genotypeCols <- vector(mode="numeric", length=0)
  if(sum(selectedSamples=="Col0_d3d")==1){genotypeCols <- append(genotypeCols, c(4, 5))}
  if(sum(selectedSamples=="hy5_d3d")==1){genotypeCols <- append(genotypeCols, c(6, 7))}
  if(sum(selectedSamples=="Col0_d3d_l1.5h")==1){genotypeCols <- append(genotypeCols, c(8, 9))}
  if(sum(selectedSamples=="hy5_d3d_l1.5h")==1){genotypeCols <- append(genotypeCols, c(10, 11))}
  return(genotypeCols)
}